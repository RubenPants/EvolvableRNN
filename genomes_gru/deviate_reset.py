...
Deviate a genome's reset weight and bias values and monitor the result.
"""
import argparse
import os
...
from genomes_gru.deviate_main import main as plot_positions, merge, get_save_path
...
def main(genome: Genome,
    ...
    """Load the genome, deviate reset gate's weights and bias and plot the results."""
    ...
    if mut_bias:
        genome_plus.nodes[gru_id].bias_h[0] += delta
        genome_minus.nodes[gru_id].bias_h[0] -= delta
    if mut_hh:
        genome_plus.nodes[gru_id].weight_hh[0, 0] += delta
        genome_minus.nodes[gru_id].weight_hh[0, 0] -= delta
    if mut_xh:
        genome_plus.nodes[gru_id].weight_xh_full[0, 0] += delta
        genome_plus.nodes[gru_id].update_weight_xh()
        genome_minus.nodes[gru_id].weight_xh_full[0, 0] -= delta
    ...
    plot_positions(
            ...
            save_name=f'reset/{name}_trajectory',
            gid=gid,
            duration=duration,
    )
...
def monitor_activation(genome: Genome, gid: int, debug: bool = False, duration: int = 60):
    """Monitor the activation of the reset gate. Note: game is started again, no worries since deterministic."""
    cfg = Config()
