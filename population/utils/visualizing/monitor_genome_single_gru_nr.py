"""
monitor_genome_simple_gru_nr.py

Monitor a single genome during its run on a single game. This monitoring focuses on the GRU-NR-cell that must be present
in the genome.
"""
import argparse
import os
...
def main(population: Population, game_id: int, genome: Genome = None, game_cfg: Config = None, debug: bool = False):
    ...
    assert len([n for n in genome.get_used_nodes().values() if type(n) == GruNoResetNodeGene]) >= 1
...
def get_gru_states(net, x):
    ...
    Z_t = sigmoid(W_xh[:, 0:1] + W_hh[:, 0:1] + gru.bias[0:1])
    H_t_tilde = np.tanh(W_xh[:, 1:2] + W_hh[:, 1:2] + gru.bias[1:2])
    H_t = (1 - Z_t) * H_t_tilde + Z_t * gru.hx
    return H_t[0, 0], H_t_tilde[0, 0], Z_t[0, 0]
